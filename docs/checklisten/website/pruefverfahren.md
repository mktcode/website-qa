...
Der strukturierte Nachweis des stabilen Basiskatalogs des HTTP-Prüfers gibt zusätzlich atomare positive, negative, nicht anwendbare und unklare Prüfaussagen aus. Er bildet deklarierte CSP, einen syntaktisch erkennbaren Framing-Schutz, `X-Content-Type-Options: nosniff`, Referrer Policy, Permissions Policy und HSTS auf dem regulären HTML, der 404-Probe und soweit entdeckt je einer bereits für MIME-, Cache- und Kompressionsprüfungen abgerufenen CSS-/JavaScript-Ressource ab. Ein fehlgeschlagener Abruf führt für die betroffene Headerabdeckung zu `inconclusive`; HSTS ist bei ausdrücklich zugelassenem HTTP `notApplicable`. Dafür werden keine zusätzlichen Anfragen gestartet.

Die bloße Deklaration belegt weder inhaltliche Stärke, widerspruchsfreie Direktiven noch risikogerechte Projekteignung. APIs, weitere Assets und öffentliche Dateien, sensible Antworten, alternative Hosts, app- und proxyseitige Redirectheader sowie die tatsächliche Anwendung-/Proxygrenze bleiben projektspezifisch zu prüfen. Die Zusammenfassung trennt vollständig belegte, teilweise belegte, fehlgeschlagene und offene Checklistenpunkte. Manuelle, redaktionelle, kommunikative, externe oder nur mit Infrastrukturzugang mögliche Kriterien bleiben ausdrücklich offen; ein erfolgreicher HTTP-Lauf schließt sie nicht ab. Da der stabile Basiskatalog bewusst nicht die gesamte Checkliste umfasst, darf seine Punktzahl nicht als Gesamtfortschritt des Websiteprojekts ausgegeben werden.

Die optionale Bibliothek `@mktcode/website-qa/report` kann gespeicherte technische JSON-Berichte mit einer projektspezifischen Modulauswahl und manuellen beziehungsweise externen Nachweisen verbinden und daraus JSON und Markdown ableiten. Sie führt selbst keine Netzwerkprüfung aus und ändert keine Projektcheckliste. Nur Berichte der festgelegten Auswertungsumgebung werden berücksichtigt; die konfigurierte Ziel-URL muss im technischen Bericht ausgewiesen sein. Querywerte und ausdrücklich erlaubte private Zielhosts werden dabei nicht protokolliert oder gebunden, sondern nur die redigierte technische Darstellung und gegebenenfalls Parameternamen verglichen. Quell- und Deploymentstand sind projektseitig deklarierte Zuordnungen. Widersprüche innerhalb dieser Angaben führen zum geschlossenen Abbruch, sie sind aber keine unabhängige Bestätigung durch das Werkzeug.

Für bewusst gestartete lokale beziehungsweise operative Prüfserien kann `writeProjectReportBundle` einen UTC-datierten Ordner unter `.website-qa/reports/` erzeugen. Er enthält die vollständigen technischen JSON-Ausgaben, `report.json`, den vollständigen `report.md` und ein Prüfsummenmanifest. `.website-qa/` wird im Zielprojekt standardmäßig ignoriert. Nur eine getrennt und restriktiv gerenderte Markdown-Zusammenfassung unter `docs/website-qa/berichte/` ist zur Versionierung vorgesehen und vor dem Commit projektspezifisch zu prüfen. Die Prüfserie ist kein automatisch bei jedem Commit erforderlicher CI-Schritt.
...
Der strukturierte Nachweis des stabilen Basiskatalogs bewertet zusätzlich Canonical-Vollständigkeit und -Konsistenz, vorhandene und eindeutige Titel beziehungsweise Meta-Beschreibungen, `lang`, die Anzahl der H1-Überschriften, Sitemap-Dateien und -Einträge, robots.txt-Referenz, Sitemap-Abdeckung, interne Seiten- und Fragmentziele sowie Status und MIME-Typ interner Ressourcen. Ein begrenzter, fehlgeschlagener oder aus Nur-Lese-Vorsicht ausgelassener Seiten-, Ressourcen- oder Sitemaplauf führt bei betroffenen Aussagen zu `inconclusive` statt zu einem stillschweigenden Erfolg. Inhaltliche Eignung, tatsächliche Sprachpassung, Überschriftenhierarchie, projektspezifische Inventarvollständigkeit, API- und Content-Negotiation-Fehler, externe Links sowie dynamische oder interaktionsabhängige Ressourcen bleiben manuelle beziehungsweise gesonderte Kriterien.

...
Der Crawler verwendet ausnahmslos GET und begrenzt Seiten, Ressourcen, Antwortgrößen, Redirects und Laufzeit. Formulare werden nur inventarisiert; ihre Actions werden nie aufgerufen, kein Formular wird abgesendet und kein Button betätigt. Externe Links werden erfasst, aber nicht abgerufen. Automatisch entdeckte interne Seiten, Sitemap-Dateien und Ressourcen mit verdächtigen Aktionspfaden oder sensitiven Query-Parametern werden ebenso wie entsprechende Redirectziele vorsorglich nicht angefordert und als ausgelassene Prüfung ausgewiesen. `--json` erzeugt eine maschinenlesbare Ausgabe mit ausdrücklichem Nachweis dieser Nur-Lese-Grenzen; `--json-file=<Pfad>` schreibt sie direkt in eine lokale Datei. Werkzeugcommit, Ziel, Optionen, Abdeckung und ausgelassene Pfade werden protokolliert.
